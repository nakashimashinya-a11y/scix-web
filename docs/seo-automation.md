...
| 日曜 06:00（ブリーフ・Telegram） | `build_brief.py` の **11 節「本文の時点が古いページ」**（`scripts/seo/stale_pages.py`・決定論）: 本文が名乗る「◯年◯月時点／版／現在／更新」のいちばん新しい日付が 180 日以上前の JA ページを、28 日クリック順に表で出す（「公表」「改定」は外部文書の日付なので見ない・時点表現の無いページは判定しない）。**週次の Claude は外部サイトに出られないので、一次資料が要る時点更新は自動でやらない**（`weekly_prompt.md` に「11 節の日付・件数は直さない」）。日曜朝の Telegram に先頭の 1 本と本数を 1 行（`weekly_run.sh` の `stale_line`）＝更新は CC の作業セッションで中島さんの指示で（2026-09-22 中島決定・選択肢①。同日の /column-jcstar・/column-lda PR #95 が型）。`最新.md` にも上位 10 本 | `weekly/<日付>/brief.md` 11 節・`brief.json` の `stale_pages` |
