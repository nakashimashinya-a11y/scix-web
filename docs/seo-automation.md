...
| 日曜 06:00（ブリーフ・Telegram） | `build_brief.py` の **11 節「アクセスが減ったページ」**（`scripts/seo/access_drop.py`・決定論）: 4 節と同じ基準（28 日クリックが前 28 日の 7 割未満・前 28 日 20 クリック以上）で減ったページに、**原因の型**（順位落ち／表示減／CTR落ち）と**本文の時点**（本文が名乗る「◯年◯月時点／版／現在／更新」の最新日付。「公表」「改定」は外部文書の日付なので見ない）と**次の手**（時点が 90 日以上前 → 本文の時点更新（CC）／CTR落ちだけ → title・description（週次）／それ以外 → 本文の受け皿を見直す（CC 判断））を添えて、失ったクリック順に表で出す。**週次の Claude は外部サイトに出られないので、一次資料が要る時点更新は自動でやらない**（`weekly_prompt.md`: CC の行の日付・件数は直さない）。日曜朝の Telegram に先頭の 1 本と本数を 1 行（`weekly_run.sh` の `drop_line`）＝時点更新は CC の作業セッションで中島さんの指示で（2026-09-22 中島決定「トリガーはアクセス減」。同日の /column-jcstar・/column-lda PR #95 が型）。`最新.md` にも上位 10 本 | `weekly/<日付>/brief.md` 11 節・`brief.json` の `access_drops` |
