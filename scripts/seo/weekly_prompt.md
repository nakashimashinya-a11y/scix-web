...
- 週次で直す候補のきっかけはアクセスの減少にし、古いことだけでは直さない `O16-68`
...
- 週次の変更は1週8件・既存12ページまでにし（迷ったら減らす）、育成は1件にまとめる `O16-36`
  ...
  - ブリーフ 11 節「アクセスが減ったページ」: 「次の手」が title・description（週次）の行は 3 の仕事として受ける。本文の時点更新（CC）と本文の受け皿を見直す（CC 判断）の行は、日付・件数・数字を直さず「◯年◯月時点」も書き換えない（O16-28）
