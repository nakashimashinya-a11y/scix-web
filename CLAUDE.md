...
- **週次自動更新が動いている**（日曜 06:00・承認なしで公開）。仕組み・止め方・戻し方は `docs/seo-automation.md`。台帳は Drive `9_システム/scix-web解析/`。自動のコミットは `auto(seo):`（週次）と `auto(structure):`（構成）、差し戻しは `git revert`。**自動の側はコラムを書かない・新しいページを作らない**（2026-09-20 中島決定「Column は僕が書く」。検査 `scripts/seo/guard_diff.py` が新規ファイルを止める。主題の提案だけ Telegram に 1 行）。自動の仕事は、アクセスと検索のデータを見て既存ページの中身・構成・流れを直し、検索からのヒットと問い合わせを増やすこと。PR で足した新規ページは翌朝、変更台帳に自動で載る（`scripts/seo/register_new_pages.py`＝手で記帳しなくてよい）。立ち上がりは週次ブリーフの 10 節に出て、週次が内部リンク・ハブカード・sitemap・`JA_ONLY_COLUMNS` の登録漏れを手当てする。**構成**（ハブの並び・トップの節の順・CTA の行き先・収益ページへの導線）は月の第 1 日曜に同じジョブが見直し、**検査を通れば承認なしで公開**する（タグ `auto/structure-YYYY-MM`・14 日後と 28 日後に効果測定・worse は週次が戻す）。**ナビ（`header.js`）を含む回だけ** 公開せず PR `auto/structure-YYYY-MM` に出す（マージで公開・閉じれば不採用。ナビの組み替えは 90 日に 1 回まで）。
