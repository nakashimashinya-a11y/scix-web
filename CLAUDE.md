...
- **週次自動更新が動いている**（日曜 06:00・承認なしで公開）。仕組み・止め方・戻し方は `docs/seo-automation.md`。台帳は Drive `9_システム/scix-web解析/`。自動のコミットは `auto(seo):`、差し戻しは `git revert`。PR で足した新規ページは翌朝、変更台帳に自動で載る（`scripts/seo/register_new_pages.py`＝手で記帳しなくてよい）。立ち上がりは週次ブリーフの 10 節に出る。
